...
/** Monday–Sunday (YYYY-MM-DD) of the week containing "today" in `tz`. */
export function currentWeekRangeInTz(tz: string, ref: Date = new Date()): { from: string; to: string } {
  const [y, m, d] = todayInTz(tz, ref).split("-").map(Number);
  const base = new Date(Date.UTC(y, m - 1, d));
  const day = base.getUTCDay();
  const mondayOffset = day === 0 ? -6 : 1 - day;
  const monday = new Date(base.getTime() + mondayOffset * 86400 * 1000);
  const sunday = new Date(monday.getTime() + 6 * 86400 * 1000);
  return { from: monday.toISOString().slice(0, 10), to: sunday.toISOString().slice(0, 10) };
}
